<x-layouts.backend-layout :breadcrumbs="$breadcrumbs">
    ...
    <x-card>
        @include('madrasafunds::pages.reports.partials.filters', [
            'action' => route('admin.madrasafunds.reports.department'),
            'show'   => ['fund', 'fund_type', 'dates'],
        ])
                ...
                <tbody>
                    @forelse($rows as $row)
                        <tr class="border-b border-gray-100 dark:border-gray-700">
                            <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ $row->department?->name ?? '—' }}</td>
