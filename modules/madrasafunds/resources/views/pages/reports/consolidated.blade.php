<x-layouts.backend-layout :breadcrumbs="$breadcrumbs">
    ...
    <x-card>
        @include('madrasafunds::pages.reports.partials.filters', [
            'action' => route('admin.madrasafunds.reports.consolidated'),
            'show'   => ['fund', 'fund_type', 'department', 'dates'],
        ])
                ...
                <tbody>
                    @forelse($receipts as $receipt)
                        <tr class="border-b border-gray-100 dark:border-gray-700">
                            ...
                            <td class="px-4 py-3 text-gray-500">{{ $receipt->department?->name ?? '—' }}</td>
                            <td class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ $receipt->student?->name ?? $receipt->donor_name ?? '—' }}</td>
                            <td class="px-4 py-3 text-right text-gray-900 dark:text-white">PKR {{ number_format((float) $receipt->amount, 2) }}</td>
                        </tr>
                    @empty
                        <tr><td colspan="6" class="text-center py-8 text-gray-500">{{ mf_bi('No records found.') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
