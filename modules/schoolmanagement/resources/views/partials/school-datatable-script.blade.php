...
class SchoolDatatable {
    ...
    _renderPagination(meta) {
        ...
        for (let i = start; i <= end; i++) {
            ...
            pages += `<button class="px-3 py-1 text-sm border rounded ${active}" onclick="window.__currentDt.goToPage(${i})">${i}</button>`;
        }
        ...
        this.paginationEl.innerHTML = `
            <div class="flex items-center justify-between gap-4 flex-wrap">
                ...
                <div class="flex items-center gap-1">
                    <button class="px-3 py-1 text-sm border rounded bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed" onclick="window.__currentDt.goToPage(${current_page - 1})" ${current_page === 1 ? 'disabled' : ''}>&laquo; Prev</button>
                    ${pages}
                    <button class="px-3 py-1 text-sm border rounded bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed" onclick="window.__currentDt.goToPage(${current_page + 1})" ${current_page === last_page ? 'disabled' : ''}>Next &raquo;</button>
                </div>
            </div>`;
    }
    ...
    static actions({ viewUrl, editUrl, deleteUrl, canEdit, canDelete, deleteConfirm }) {
        ...
        if (canDelete && deleteUrl) {
            const confirmMessage = (deleteConfirm || 'Delete this record?').replace(/'/g, "\\'");
            html += `<button type="button" class="btn btn-danger btn-xs" title="Delete" onclick="SchoolDatatable.deleteRecord('${deleteUrl}', '${confirmMessage}')"><iconify-icon icon="lucide:trash-2" width="14"></iconify-icon></button>`;
        }
        html += '</div>';
        return html;
    }

    static async deleteRecord(url, confirmMessage) {
